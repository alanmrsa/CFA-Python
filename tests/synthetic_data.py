...
def gen_data(n_samples, X_func, Z_func, W_func, Y_func, pux, puz, puw, puy, puxz): 
    # define the SFM
    X = X_func
    Z = Z_func
    W = W_func
    ...
    #generate U's
    U_x = np.random.binomial(1, pux, n_samples)
    U_z = np.random.binomial(1, puz, n_samples)
    U_w = np.random.binomial(1, puw, n_samples)
    U_y = np.random.binomial(1, puy, n_samples)
    U_xz = np.random.binomial(1, puxz, n_samples)
...
def compute_true_effects(pux, puz, puw, puy, puxz, X, Z, W, Y):
    NDE = 0
    NIE = 0
    ExpSE_x0 = 0
    ExpSE_x1 = 0
