...
def causal_loss(pred, pred0, pred1, X, Z, W, Y, px_z, eta_de, eta_ie, eta_se_x1, 
                eta_se_x0, relu_eps, eps, task_type, return_path_effects=False):
    ...
    if return_path_effects:
      return nde_loss, nie_loss, nse_loss_x1, nse_loss_x0
    return custom_loss

def train_w_fioretta(train_data, eval_data, x_col, z_cols, w_cols, y_col, step_sizes = [1], lr=0.001,
    ...
    while restarts <= max_restarts:
        ...
        for epoch in range(epochs):
            ...
            # Get step size for this epoch
            if epoch < len(step_sizes):
                step_size = step_sizes[epoch]
            else:
            ...
            for i in range(0, fts_train_t.size()[0], batch_size):
                ...
                custom_loss = causal_loss(pred, pred0, pred1,
                                          ...
                                          eta_se_x1, eta_se_x0,
                                          relu_eps, eps, task_type, return_path_effects=True)
                
                ndeL, nieL, nse0L, nse1L = custom_loss
                causal_l = lmbd_nde* ndeL +  lmbd_nie* nieL +  lmbd_nse_x0* nse0L +  lmbd_nse_x1 * nse1L
                total_loss = bce_loss + causal_l
                nde_vio.append(ndeL.item())
                nie_vio.append(nieL.item())
                nse0_vio.append(nse0L.item())
                nse1_vio.append(nse1L.item())

                optim.zero_grad()
                total_loss.backward()
                optim.step()
            
            lmbd_nde+= step_size* np.mean(nde_vio)
            lmbd_nie+= step_size* np.mean(nie_vio)
            lmbd_nse_x0+= step_size* np.mean(nse0_vio)
            lmbd_nse_x1+= step_size* np.mean(nse1_vio)

            nde_lmbd_hist.append(lmbd_nde)
            nie_lmbd_hist.append(lmbd_nie)
            nse0_lmbd_hist.append(lmbd_nse_x0)
            nse1_lmbd_hist.append(lmbd_nse_x1)
            ...
            with torch.no_grad():
                ...
                eval_tot_loss = eval_bce_loss +  (eval_causal_loss[0] * lmbd_nde + eval_causal_loss[1] * lmbd_nie+  eval_causal_loss[2] * lmbd_nse_x0 +  eval_causal_loss[3] * lmbd_nse_x1)
                loss_hist.append(eval_tot_loss.item()) 
                nde_hist.append(eval_causal_loss[0].item())
                nie_hist.append(eval_causal_loss[1].item())
                nse0_hist.append(eval_causal_loss[2].item())
                nse1_hist.append(eval_causal_loss[3].item())
    ...
    return (best_model_global, loss_hist, nde_hist, nie_hist, nse0_hist, nse1_hist, nde_lmbd_hist, nie_lmbd_hist, nse0_lmbd_hist, nse1_lmbd_hist)
